...
function formatarPedido(row, itens = []) { // Recebe row e um array com os itens selecionados
  ...
  return {
    ...
    itens: itens.map(it => ({
      _id:           it.id,
      embalagem:    it.peca_id,
      nomeEmbalagem: it.nome_peca,
...
const Pedido = { // Variável pedido recebe:
  ...
  async create({ clienteId, itens, taxaEntrega = 0, formaPagamento, troco = 0, observacoes = '', setor = null, origem = 'balcao', gestorId = null }) {
    ...
    const Embalagem = require('./Embalagens');
    let subtotal = 0;
    const itensProcessados = [];

    for (const item of itens) {
      const embalagemId = item.embalagem;
      const embalagem = await Embalagem.findById(embalagemId);
      if (!embalagem) throw new Error(`Embalagem ID ${embalagemId} não encontrada`);

      const tamanho = (item.tamanho || 'P').toUpperCase();
      const precos = embalagem.precos || {};
    ...
    for (const it of itensProcessados) {
      run(`
        ...
        VALUES (?, ?, ?, ?, ?, ?)
      `, [pedidoId, it.embalagemId, it.nomeEmbalagem, it.quantidade, it.precoUnitario, it.subtotal]);
    }
