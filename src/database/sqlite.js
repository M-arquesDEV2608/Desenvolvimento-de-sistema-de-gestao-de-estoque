...
const ready = (async () => {
    ...
    // Compatibilidade com bancos criados pela versão anterior.
    const colunasPedidos = db.exec('PRAGMA table_info(pedidos)')[0]?.values || [];
    const temUpdatedAt = colunasPedidos.some(coluna => coluna[1] === 'updated_at');
    if (!temUpdatedAt) {
        db.run('ALTER TABLE pedidos ADD COLUMN updated_at TEXT');
        db.run("UPDATE pedidos SET updated_at = update_at WHERE updated_at IS NULL");
    }
