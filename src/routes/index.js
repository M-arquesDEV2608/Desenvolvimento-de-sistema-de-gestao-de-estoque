...
const Embalagens = require('../models/Embalagens');
...
router.get('/embalagens', auth, async (req, res) => {
    try { res.json(await Embalagens.findAll())}
    catch (e) { res.status(500).json({ erro: e.message}); }
});

router.get('/embalagens/:id', auth, async (req, res) => {
    try {
        const embalagem = await Embalagens.findById(req.params.id);
        if (!embalagem) return res.status(404).json({ erro: 'Embalagem não encontrada'});
        res.json(embalagem);
    } catch (e) { res.status(500).json({ erro: e.message }); }
});
...
router.post('/embalagens', auth, async (req, res) => {
    try {
        if (!req.body.nome)
            return res.status(400).json({ erro: 'Nome é obrigatório' });
        res.status(201).json(await Embalagens.create(req.body));
    } catch (e) { res.status(500).json({ erro: e.message }); }
});

router.put('/embalagens/:id', auth, async (req, res) => {
    try {
        const embalagem = await Embalagens.update(req.params.id, req.body);
        if (!embalagem) return res.status(404).json({ erro: 'Embalagem não encontrada' });
        res.json(embalagem);
    } catch (e) { res.status(500).json({ erro: e.message }); } 
});

router.delete('/embalagens/:id', auth, async (req, res) => {
    try {
        const ok = await Embalagens.delete(req.params.id);
        if (!ok) return res.status(404).json({ erro: 'Embalagem não encontrada'});
        res.json({ mensagem: 'Embalagem deletada'});
    } catch (e) { res.status(500).json({ erro: e.message }); }
});
