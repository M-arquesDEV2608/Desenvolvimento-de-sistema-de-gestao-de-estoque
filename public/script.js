...
function aplicarPerfil(usuario) {
  ...
  show('btn-nova-embalagem', true, 'inline-flex');
...
async function carregarDashboard() {
  ...
  try {
    const [embalagens, clientes, pedidos] = await Promise.all([
      api('GET', '/embalagens'),
      api('GET', '/clientes'),
      api('GET', '/pedidos'),
    ]);

    cEmbalagens = embalagens;
    cClientes = clientes;

    document.getElementById('s-piz').textContent = embalagens.length;
    ...
    elC.innerHTML = embalagens.filter(p => p.disponivel).slice(0, 8).map(p => `
      <div class="mini-row">
        <span>🛰️ ${p.nome}</span>
        <small style="color:var(--muted)">${R$(p.precos?.G || p.precos?.P || 0)}</small>
      </div>`).join('') ||
      '<div class="empty"><span class="ei">⚙️</span>Nenhuma embalagem</div>';
...
async function abrirPedidosetor(setorNum = null) {
  try {
    if (!cEmbalagens.length) cEmbalagens = await api('GET', '/embalagens');
...
function addItemsetor() {
  ...
  const opts = cEmbalagens.filter(p => p.disponivel)
...
async function carregarEmbalagens() {
  const el = document.getElementById('tbl-embalagens');
  el.innerHTML = '<div class="spin-wrap"><div class="spin"></div> Carregando...</div>';
  try {
    cEmbalagens = await api('GET', '/embalagens');
    if (!cEmbalagens.length) {
      el.innerHTML = '<div class="empty"><span class="ei">⚙️</span>Nenhuma embalagem</div>';
      return;
    }
    el.innerHTML = `
      <table>
            ...
            <tr>
              ...
              <td>
                <div style="display:flex;gap:5px">
                  <button class="btn btn-ghost btn-sm" onclick="editarEmbalagem('${p._id}')">✏️ Editar</button>
                  <button class="btn btn-danger btn-sm" onclick="deletarEmbalagem('${p._id}','${p.nome.replace(/'/g,"\\'")}')">🗑️</button>
                </div>
              </td>
            </tr>`).join('')}
        </tbody>
...
function abrirEmbalagem() {
  document.getElementById('m-embalagem-t').textContent = 'Nova Embalagem';
...
async function editarEmbalagem(id) {
  let p = cEmbalagens.find(x => String(x._id) === String(id));

  if (!p) {
    try {
      p = await api('GET', '/embalagens/' + id);
    } catch (e) {
      toast('Erro ao carregar embalagem: ' + e.message, 'err');
      return;
    }
  }

  document.getElementById('m-embalagem-t').textContent = 'Editar Embalagem';
...
async function salvarEmbalagem() {
  ...
  try {
    if (id) {
      await api('PUT', '/embalagens/' + id, d);
      toast('Embalagem atualizada! ✅');
    } else {
      await api('POST', '/embalagens', d);
      toast('Embalagem criada! ✅');
    }
    fechar('m-embalagem');
    cEmbalagens = [];
    carregarEmbalagens();
...
async function deletarEmbalagem(id, nome) {
  if (!confirm(`Deletar "${nome}"?`)) return;
  try {
    await api('DELETE', '/embalagens/' + id);
    toast('Embalagem deletada!');
    cEmbalagens = [];
    carregarEmbalagens();
  } catch (e) { toast('Erro: ' + e.message, 'err'); }
}
          ...
          ${pedidos.map(p => `
            <tr>
              ...
              <td style="font-size:.76rem">
                ${p.itens.map(it =>
                  `${it.quantidade}x ${it.nomeEmbalagem || '?'}${it.tamanho ? ` (${it.tamanho})` : ''} — ${R$(it.precoUnitario ?? it.preco_unitario ?? 0)}`
                ).join('<br>')}
              </td>
              <td>${R$(p.subtotal)}</td>
